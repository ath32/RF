...
fasta = os.path.abspath("./eukaryotes/FASTA/Homo_sapiens.GRCh38.cds.all.fa")
...
def main():
    ...
    for seq in clean_seqs:
        gc3_list.append(float(GC123(seq)[3]))
    ...
    for ind, bin in enumerate(bins):
        ...
        mean_gc, mean_gc3 = get_gc(bin)
        taa, tga, tag = get_stop_usage(bin)

        print ('Group:', int(ind)+1, 'GC3:', mean_gc3, 'TAA:', taa, 'TGA:', tga, 'TAG:', tag)
